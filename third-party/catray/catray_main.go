...
func (x *CoreController) StopLoop() error {
	...
	if x.IsRunning {
		x.doShutdown()
		x.CallbackHandler.OnEmitStatus(0, "core stopped :3")
	}
	return nil
}
...
func (x *CoreController) doStartLoop(configContent string) error {
	log.Println("initializing core... :3")
	...
	x.CallbackHandler.OnEmitStatus(0, "Started successfully, running :3")

	log.Println("Starting core successfully :3")
	return nil
}
...
func measureInstDelay(ctx context.Context, inst *core.Instance, url string) (int64, error) {
	...
	if url == "" {
		url = "https://204.lain1.dev/generate_204"
	}
	...
	for i := 0; i < attempts; i++ {
		...
		if _, err := io.Copy(io.Discard, resp.Body); err != nil {
			lastErr = fmt.Errorf("OH NO! failed to read response body: %w", err)
			continue
		}
